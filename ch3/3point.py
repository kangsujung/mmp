...
#1
gray=cv.imread('soccer.jpg', cv.IMREAD_GRAYSCALE) # BGR 컬러 영상을 명암 영상으로 변환하여 저장->아예 그레이만 사용가능/둘 중에 하나 선택해서 사용

if img is None:
    sys.exit('파일을 찾을 수 없습니다.')

#gray=cv.cvtColor(img,cv.COLOR_BGR2GRAY)	# BGR 컬러 영상을 명암 영상으로 변환->컬러이미지 먼저 읽고 그레이로 변환
gray_small=cv.resize(gray,dsize=(0,0),fx=0.25,fy=0.25) #영상 1/4로 줄임

cv.imshow('soccer-gray',gray_small)
print(gray_small.shape)

#2 사칙연산 - opencv 함수
img_plus = cv.add(gray_small, 50)         # y = x + 50 배열(배열하나하나)+숫자 같은 크기 아웃풋
...
#3 사칙연산 - 연산자
img_plus2 = gray_small+50       # y = x + 50 255넘어서 이상하게 나옴
img_minus2 = gray_small-50      # y = x - 50
img_multi2 = gray_small*2       # y = 2 * x
img_div2 = gray_small/2         # y = x / 2
pp=np.hstack((img_plus2, img_minus2, img_multi2, img_div2))
#cv.imshow('test-plus',pp)
...
#6
img512=cv.resize(img,dsize=(512,512)) #크기 지정
opencv_img=cv.imread('opencv_logo512.png')
img_plus3 = cv.subtract(img512, opencv_img) #255초과 로그 색깔 있는 곳에 사진이 보임
cv.imshow('two images - add',img_plus3)

#7 숫자 초과안하게
img_plus4 = cv.addWeighted(img512, 0.5, opencv_img, 0.5, 0) #img256 * 0.5 + opencv_img *0.5 +0 /비율의 합은 1로 동일해야함 ex)0.7-0.3
