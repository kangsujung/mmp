...
class TrafficWeak(QMainWindow):
    def __init__(self):
        ...
        self.signImgs=[]				# 표지판 모델 영상 저장 / 이미지배열 만듬
        
    def signFunction(self):
        self.label.clear()
        self.label.setText('교통약자 번호판을 등록합니다.')
        
        for fname,_ in self.signFiles: #3개의 파일 읽음
            self.signImgs.append(cv.imread(fname)) #현재배열의 가장 마지막에 추가한다
            cv.imshow(fname,self.signImgs[-1])   #-1은 가장 마지막에 있는 이미지 보여주는 것
    ...
    def roadFunction(self):
        if self.signImgs==[]: 
            ...
            self.roadImg=cv.imread(fname[0]) #읽으려는 이미지 저장
    ...
    def recognitionFunction(self):
        if self.roadImg is None: 
            ...
            for img in self.signImgs: #각각 이미지 그레이로 바꾸고 kd배열에 저장
            ...
            if len(GM[best])<4:	# 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                box1=np.float32([[0,0],[0,h1-1],[w1-1,h1-1],[w1-1,0]]).reshape(4,1,2) #4개의 꼭짓점
